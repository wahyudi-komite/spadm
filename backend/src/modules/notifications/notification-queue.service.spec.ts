...
describe('NotificationQueueService', () => {
  ...
  const mockDelivery = {
    id: 1, notificationId: 1, channel: 'WHATSAPP', recipient: '628123456789',
    template: 'TEST', payload: { message: 'Hello' },
    status: DeliveryStatus.PENDING, attempts: 0,
    nextAttemptAt: null, lastError: null,
    providerMessageId: null, sentAt: null, deliveredAt: null,
  ...
  describe('processQueue', () => {
    it('should process pending deliveries', async () => {
      ...
      expect(whatsappProvider.sendText).toHaveBeenCalledWith('628123456789', 'Hello');
      expect(deliveryRepo.update).toHaveBeenCalledWith(1, {
        status: DeliveryStatus.SENT, providerMessageId: 'msg-123',
        sentAt: expect.any(Date), lastError: null,
      });
    });
    ...
    it('should handle send failure with retry', async () => {
      ...
      expect(deliveryRepo.update).toHaveBeenCalledWith(1, {
        status: DeliveryStatus.RETRY, lastError: 'Network error',
        nextAttemptAt: expect.any(Date),
      });
    });

    it('should mark as failed after max retries', async () => {
      const retryDelivery = { ...mockDelivery, attempts: 5, status: DeliveryStatus.RETRY } as NotificationDelivery;
      ...
      expect(deliveryRepo.update).toHaveBeenCalledWith(1, {
        status: DeliveryStatus.FAILED, lastError: 'Final error',
        nextAttemptAt: null,
      });
    });
    ...
    it('should skip delivery claimed by another instance', async () => {
      deliveryRepo.find.mockResolvedValue([mockDelivery]);
      deliveryRepo.update.mockResolvedValueOnce({ affected: 0, raw: [] } as any);
